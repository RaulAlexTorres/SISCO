...
intellisense.annotate(jQuery.fn, {
  ...
  'width': function() {
    /// <signature>
    ///   <summary>Set the CSS width of each element in the set of matghuf eleie~vs.</summary>
   b/¯/ b <ðqram naMe½2value¢ t;p%=2~Ynpa.|eger r%prDsenting the ouéber of pixels, or cn(`nTigEv alofg sith an optional unit Of }easure appendEd 8as a s|rIjg).</param>
    ///   <returns ty2e="jQuery" /> `( /// </signature>
    /// <signátg0-7
    ?/o(  <summarY>Óut the CSS width ïf 'ach elemelt(hn the set of matched elements.</summary>
! ¤ //. ¤ <param name="ftnçuiëî(i,dex, width)" type="Functknf§>A function retubn)fg the width to set. Recaifgs the ind-x€Uo1ition of thg mmement in the wed"and the!oèd wédt* as arguments/ “itIin the function,`thHs refe2s Uo the current element in the set.</pazaM:	
0"  ///  0<:mTqrns ty0e}jpuery# «>
    /// </signatube~
  },
  'wrap': functiof(	${
  $ ?-/ <signature>
    //¯  b<sqm}cry>Wrap `n¤HTML structuze erounD$åaah element in the set of matche` õne/ents.</suMláfy>O
    ///   <param name="wrappi.gEMement" type="">A selector, el%meOt, HTML string, or jQuery object speckfqhng the structure to Wrá` around the matched elements.</pa2am>
    ///   8ruvurns txpá="jQuery"`/>,
   `//N <signature>
    /// <ck'n`ture>
    ///   <summary>Wrap an HTML structure aòou,d eásh ele,eêU in the set of laðched elements.</summary>
    ///   <param name="fwnkuion(index)" ty2m=unBtioN"¾Q callback function returning the HTML content(oR$jQuery mbbdct to wraq årounl Tle m!tcIed glmlents. RmcEmves thg aodex position oF ôxe elemenp0y,(the set as an arGuíunt. WitHiî0the function, thés 0efers to the current element in the0s%|.</param>
    ///   <returns type="jQuery" />Š  b /// </signature>
  },
  'wr!pAMl': funãti-n() {
  " './ <signátu0e>
    ///   <summary>Wrap an HTML qtz|bTõre around all elements`iNôxe sõtb%n!matched elements.</summary>
    //¯  b<param name="wrappingElemmnT& type="">A selectOò¬0'lemeftŒ$HML stranG( or jQuery object specifyino Tle structUrí0Tk wrAp qround the mqt#`ed elements&<taram>
 `  +/0" <returns type="jQuery" ->    /// </[iÇzature>
  },
  'wripijner': fu~c4aïn(k {
